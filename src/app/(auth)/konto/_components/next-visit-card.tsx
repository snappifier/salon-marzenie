...
export function NextVisitCard({booking}: Props) {
	...
	return (
		<div className="bg-gradient-to-b from-surface-muted to-surface border border-accent-100 rounded-[var(--radius-lg)] p-7 relative overflow-hidden">

			<div className="grid grid-cols-1 sm:grid-cols-[auto_1fr] gap-7 sm:gap-8 relative">
				<div className="flex flex-col">
					<p className="text-[10px] font-medium uppercase tracking-[0.18em] text-interactive mb-2">
						Najbliższa wizyta
					</p>
					<p className="font-display font-medium text-[clamp(36px,4.5vw,48px)] leading-none text-primary tracking-[-0.025em] mb-1">
						{day} <em className="italic text-interactive font-normal">{month}</em>
					</p>
					<p className="text-sm text-secondary mb-3">
						{weekday}, {year}
					</p>
					<p className="font-display font-medium text-[22px] text-interactive-hover mb-1">{time}</p>
				...
				<div className="border-t border-border-subtle pt-5 sm:border-t-0 sm:pt-0 sm:border-l sm:border-border-subtle sm:pl-7">
					<p className="text-[11px] font-medium uppercase tracking-[0.18em] text-secondary mb-3">
						Zaplanowane zabiegi
					</p>
					...
					{booking.items.map((item) => (
						<div
							key={item.id}
							className="flex justify-between items-baseline py-2.5 border-b border-border-subtle last:border-b-0"
						>
							<div>
								<p className="font-display font-medium text-[15px] text-primary tracking-[-0.01em]">
									{item.serviceName}
							...
							<p className="font-display font-medium text-sm text-primary shrink-0">
								{formatMoney(item.priceGr)}
							</p>
						</div>
					))}

					<div className="flex justify-between items-baseline pt-3.5 mt-1.5 border-t border-accent-100">
						<p className="text-xs text-secondary uppercase tracking-[0.18em] font-medium">Razem</p>
						<p className="font-display font-medium text-[22px] text-primary tracking-[-0.01em]">
							{formatMoney(booking.totalPriceGr)}
						</p>
					</div>

					<div className="flex items-center gap-2 mt-[18px] px-3.5 py-2.5 bg-surface/60 rounded-[var(--radius-md)] text-xs text-secondary">
						<span className="text-interactive shrink-0">
							<Clock size={14} strokeWidth={1.8} />
						</span>
						Bezpłatne odwołanie do{" "}
						<strong className="text-primary font-medium">{cancelDeadlineLabel}</strong>
					</div>
	...
	return (
		<div className="bg-gradient-to-b from-surface-muted to-surface border border-accent-100 rounded-[var(--radius-lg)] p-7 relative overflow-hidden">
			...
			<div className="relative">
				...
				<p className="font-display font-medium text-[clamp(24px,3vw,32px)] leading-tight text-primary tracking-[-0.02em] mb-2">
					Brak zaplanowanej wizyty.{" "}
					<em className="italic text-interactive font-normal">Zarezerwujmy nową.</em>
				</p>
				<p className="text-sm text-secondary max-w-[420px] mb-5">
